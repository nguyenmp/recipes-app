...
const ARCHIVE_BOX_API_KEY = process.env.ARCHIVE_BOX_API_KEY!;
const ARCHIVE_BOX_URL = process.env.ARCHIVE_BOX_URL!;
const ARCHIVE_BOX_HOST = process.env.ARCHIVE_BOX_HOST!;
...
assert(JSON.parse(await post(`${ARCHIVE_BOX_URL}/api/v1/auth/check_api_token`, {token: ARCHIVE_BOX_API_KEY}, {Host: ARCHIVE_BOX_HOST}))['success']);
...
export async function resetDatabaseTables() {
    ...
    await sql`DROP TABLE IF EXISTS LinkContent CASCADE`;
    ...
    await sql`CREATE TABLE IF NOT EXISTS LinkContents (link_id BIGINT NOT NULL REFERENCES Links(id), link_content TEXT NOT NULL)`;
...
export async function addLinksForNote(note_id: number, note: ShallowNote): Promise<void> {
    ...
    for (const link of links) {
        ...
        const result = await sql<{id: number}>`INSERT INTO Links (url) VALUES (${url}) ON CONFLICT DO NOTHING RETURNING id`;
